...
def autoplay_audio(audio_bytes, audio_id=None):
    ...
    last_played = st.session_state.get("last_played_audio_id")

    if audio_id and last_played == audio_id:
        return
    ...
    components.html(
        ...
        (function() {{
            ...
            if (parentWin._playCoachBase64Audio) {{
                parentWin._playCoachBase64Audio("{b64}");
            }} else {{
                var audio = new Audio("data:audio/mp3;base64,{b64}");
                audio.play().catch(function(err) {{
                    console.warn("Direct HTML Audio play blocked:", err);
                }});
            }}
        }})();
        </script>
