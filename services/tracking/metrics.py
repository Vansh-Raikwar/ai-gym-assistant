...
def sync_metrics_update(context):
    ...
    last_saved_reps = st.session_state.get("last_saved_reps", 0)
    last_saved_sets = st.session_state.get("last_saved_sets_completed", 0)

    if reps > last_saved_reps:
        newly_saved_reps = reps - last_saved_reps
        newly_completed_sets = max(sets_completed - last_saved_sets, 0)
        now_ts = time.time()
        ...
        if newly_completed_sets > 0 and st.session_state.get("voice_pipeline"):
            ...
            _store_voice_result(result)

        st.session_state.set_cycle_started_at = now_ts
        st.session_state.last_saved_reps = reps
